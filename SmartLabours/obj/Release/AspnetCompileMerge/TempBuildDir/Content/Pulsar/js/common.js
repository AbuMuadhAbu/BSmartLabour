...
var ajaxUrl = $('#ClientSubClientID').attr("data-ajax-url").toString();
var baseUrl = ajaxUrl.substr(0, ajaxUrl.indexOf("-Controller"));
$(function () {
    ...
    $(".topnav").accordion({
        ...
        speed: "fast",
        closedSign: '',
        openedSign: '<i class="icn-active"></i>'
    });
...
$(document).ajaxError(function (event, jqxhr, settings, exception) {
    ...
    if (jqxhr.responseJSON != undefined) {
        if (jqxhr.responseJSON.block != undefined) {
            window.location.pathname = baseUrl + 'DumpUploadBlock/DumpUploading/' + jqxhr.responseJSON.block;
        }
    }
    else if ($.parseJSON(settings.data) != undefined) {
        var errString = "";
        $.each($.parseJSON(settings.data), function (variable, value) {
            errString += variable + ' : ' + value + '  ';
        });
        errString += " ; URL: " + (settings.url ? settings.url : "undefined");
        Callback("Dashboards", "ErrorWrite", function () { }, function () { }, "Message", errString);
        //if ($.parseJSON(settings.data).DumpID != undefined) {
        //    window.location.pathname = baseUrl + 'DumpUploadBlock/DumpUploading/' + $.parseJSON(settings.data).DumpID;
        //}
        //else if ($.parseJSON(settings.data).dumpID != undefined) {
        //    window.location.pathname = baseUrl + 'DumpUploadBlock/DumpUploading/' + $.parseJSON(settings.data).dumpID;
        //}
    }
});
